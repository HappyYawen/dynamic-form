<template>
  <div id="myForm">
    <Row>
      <Col span="8" style="border-right:1px solid #eee;padding-right:20px;">
        <h3 style="text-align:left;">新增表单:</h3>
        <!--新增表单规则-->
        <Form id="newForm" ref="newForm" label-position="right" :model="oneField" :label-width="85">
          <!-- <Row class="add-form">
            ...
            <Col span="4"> -->
              <Form-item label="类型" prop="type" :rules="{required:true,message: '不能为空', trigger: 'change'}">
                <Select v-model="oneField.type" placeholder="请选择" @on-change="resetRegexp">
                  <Option v-for="(value,key) in selectMess" :value="key" :key="key" :label="value"></Option>
                </Select>
              </Form-item>
            <!-- </Col>
            <Col span="4"> -->
              <Form-item label="文本域行数" prop="rows"  v-show="oneField.type=='textarea'"
              :rules="{required:true,type:'integer',message: '请输入数字', trigger: 'blur'}">
            ...
            <!-- </Col>
            <Col span="4" > -->
              <Form-item prop="rules" label="验证规则" v-show="oneField.type=='input'">
                <Select placeholder="请选择" v-model="oneField.rules" @on-change="selectRules">
                  <Option v-for="(value,key) in typeMess" :value="key" :key="key" :label="value"></Option>
                </Select>
              </Form-item>
            <!-- </Col>
            <Col span="2" offset="1"> -->
      ...
      <Col span="16" style="padding-left:20px;">
        <h3 style="text-align:left;">表单预览:</h3>
        <!--已存在的表单-->
            ...
            <Row>
              ...
              <!--如果是select-->
                <Select v-if="item.type=='select'" v-model="item.value">
                  <Option v-for="(optionItem, childIndex) in item.option" :value="optionItem.value" :key="childIndex">{{optionItem.text}}</Option>
                </Select>
              <!--如果是checkbox-->
                <Checkbox-group v-if="item.type=='checkbox'" v-model="item.value">
                  <Checkbox v-for="(optionItem, childIndex) in item.option"  :key="childIndex" :label="optionItem.value">
                    <span>{{optionItem.text}}</span>
                  </Checkbox>
                </Checkbox-group>
              <!--如果是radio-->
                <Radio-group v-if="item.type=='radio'" v-model="item.value">
                  <Radio v-for="(optionItem, childIndex) in item.option"  :key="childIndex" :label="optionItem.value">
                    <span>{{optionItem.text}}</span>
                  </Radio>
                </Radio-group>
              <!--如果是日期-->
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd () {
        //先验证新增表单规则是否正确。
        let valid = 1;
        this.$options.methods.handleValidate.call(this,'newForm');
        if(this.$refs.regexpForm.$el.style.display=="show"){
          this.$options.methods.handleValidate.call(this,'regexpForm');
          valid+=1;
        }
        if(this.$refs.muDynamic.$el.style.display=="show"){
          this.$options.methods.handleValidate.call(this,'muDynamic');
          valid+=1;
        }
        ...
        if(this.isValid==valid){
          ...
          //一条rule
          if(["integer","float","number"].includes(field.rules)){
      ...
      resetRegexp () {
        if(!this.oneField.type||this.noMuli.includes(this.oneField.type)){
          this.isMuli= false;
        }else{
          this.isMuli= true;
        }
